<template>
  <div class="theme-styles-extract">
    ...
    />
    <div class="content">
      <div v-if="themeStyles.fontNames.length" class="config-item">
      ...
      <div v-if="themeStyles.fontColors.length" class="config-item">
          ...
          >
            <div
              ...
              :style="{
                ...
                color: getMostReadableColor(item),
              }"
            >
              {{ getHexColor(item) }}
            </div>
      ...
      <div v-if="themeStyles.backgroundColors.length" class="config-item">
      ...
      <div v-if="themeStyles.themeColors.length" class="config-item">
        <div class="label">
          主题色：<span class="tip">（点击色块排除不要的颜色）</span>
        </div>
...
const removeThemeColor = (index: number) => {
  if (selectedIndex.value.themeColors.includes(index)) {
    selectedIndex.value.themeColors = selectedIndex.value.themeColors.filter(
      (i) => i !== index
    )
...
const getMostReadableColor = (color: string) => {
  ...
  return tinycolor
    .mostReadable(color, colorList, { includeFallbackColors: true })
    .toRgbString()
}
