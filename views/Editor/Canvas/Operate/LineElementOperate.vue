...
const svgWidth = computed(() =>
  Math.max(props.elementInfo.start[0], props.elementInfo.end[0])
)
const svgHeight = computed(() =>
  Math.max(props.elementInfo.start[1], props.elementInfo.end[1])
)
...
const resizeHandlers = computed(() => {
  ...
  if (
    props.elementInfo.curve ||
    props.elementInfo.broken ||
    props.elementInfo.broken2
  ) {
