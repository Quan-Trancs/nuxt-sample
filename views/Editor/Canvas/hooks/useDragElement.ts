...
export default (
  ...
  const { activeElementIdList, activeGroupElementId } = storeToRefs(
    useMainStore()
  )
  ...
  const dragElement = (e: MouseEvent | TouchEvent, element: PPTElement) => {
    ...
    const originElementList: PPTElement[] = JSON.parse(
      JSON.stringify(elementList.value)
    )
    ...
    const elOriginHeight =
      'height' in element && element.height ? element.height : 0
    const elOriginRotate =
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      for (let i = 0; i < horizontalLines.length; i++) {
        ...
        if (
          Math.abs(targetMinY - value) < sorptionRange &&
        ...
        if (
          Math.abs(targetMaxY - value) < sorptionRange &&
          !isHorizontalAdsorbed
        ) {
        ...
        if (
          Math.abs(targetCenterY - value) < sorptionRange &&
          !isHorizontalAdsorbed
        ) {
      ...
      for (let i = 0; i < verticalLines.length; i++) {
        ...
        if (
          Math.abs(targetMinX - value) < sorptionRange &&
          !isVerticalAdsorbed
        ) {
        ...
        if (
          Math.abs(targetMaxX - value) < sorptionRange &&
          !isVerticalAdsorbed
        ) {
        ...
        if (
          Math.abs(targetCenterX - value) < sorptionRange &&
          !isVerticalAdsorbed
        ) {
      ...
      // 其他被选中的元素的位置信息通过正在操作的元素的移动偏移量来进行计算
        ...
        const handleElement = elementList.value.find(
          (el) => el.id === element.id
        )
