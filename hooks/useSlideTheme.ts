        ...
        if (el.type === 'shape' || el.type === 'text') {
          ...
          for (const row of el.data) {
            for (const cell of row) {
              ...
              if (cell.text) {
                const percent =
                  cell.text.length >= 10 ? 1 : cell.text.length / 10
                if (cell.style?.color) {
                  fontColorValues.push({
  ...
  const getSlideAllColors = (slide: Slide) => {
    ...
    for (const el of slide.elements) {
      ...
      if (
        el.type === 'image' &&
        el.colorMask &&
        tinycolor(el.colorMask).getAlpha() !== 0
      ) {
      ...
      if (
        el.type === 'table' &&
        el.theme &&
        tinycolor(el.theme.color).getAlpha() !== 0
      ) {
