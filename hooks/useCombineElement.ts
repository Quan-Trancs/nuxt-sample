...
export default () => {
  ...
  const { activeElementIdList, activeElementList, handleElementId } =
  ...
  const uncombineElements = () => {
    if (!activeElementList.value.length) return
    const hasElementInGroup = activeElementList.value.some(
      (item) => item.groupId
    )
    ...
    const handleElementIdList = handleElementId.value
