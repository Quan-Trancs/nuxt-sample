...
export default () => {
  ...
  const highlightTableText = (nodes: NodeListOf<Element>, index: number) => {
    for (const node of nodes) {
      node.innerHTML = node.innerHTML.replace(
        new RegExp(searchWord.value, modifiers.value),
        () => {
  ...
  const searchNext = () => {
    ...
    else if (searchIndex.value < searchResults.value.length - 1)
      searchIndex.value += 1
      ...
      if (mark.classList.contains('active')) {
        if (replaced) parentNode.removeChild(mark)
          ...
          parentNode.replaceChild(
            document.createTextNode(replaceWord.value),
            mark
          )
          replaced = true
        }
  ...
  const replaceAll = () => {
    ...
    for (let i = 0; i < searchResults.value.length; i++) {
      ...
      const targetSlide = slides.value.find(
