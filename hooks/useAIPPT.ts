...
export default () => {
  ...
  const getUseableTemplates = (
    ...
    if (n === 1) {
      const list = templates.filter((slide) => {
        ...
        const texts = slide.elements.filter((el) =>
          checkTextType(el, 'content')
        )
    ...
    if (list.length === 0) {
      ...
      target = list.reduce((closest, current) => {
        const currentLen = current.elements.filter((el) =>
          checkTextType(el, type)
        ).length
        const closestLen = closest.elements.filter((el) =>
          checkTextType(el, type)
        ).length
    ...
    return templates.filter((slide) => {
      const len = slide.elements.filter((el) => checkTextType(el, type)).length
      const targetLen = target!.elements.filter((el) =>
        checkTextType(el, type)
      ).length
  ...
  const getNewTextElement = ({
    ...
    content = doc.body.innerHTML.replace(
      /font-size:(.+?)px/g,
      `font-size: ${size}px`
    )
  ...
  const getUseableImage = (el: PPTImageElement): ImgPoolItem | null => {
    ...
    if (el.width === el.height)
      imgs = imgPool.value.filter((img) => img.width === img.height)
    ...
    for (const template of _AISlides) {
      if (template.type === 'content') {
        ...
        if (items.length === 5 || items.length === 6) {
          ...
          AISlides.push({
            ...template,
            data: { ...template.data, items: items1 },
          })
          AISlides.push({
            ...template,
            data: { ...template.data, items: items2 },
            offset: 3,
          })
          ...
          AISlides.push({
            ...template,
            data: { ...template.data, items: items3 },
            offset: 6,
          })
    ...
    for (const item of AISlides) {
      if (item.type === 'cover') {
        ...
        const contentsTemplate =
          _contentsTemplates[
            Math.floor(Math.random() * _contentsTemplates.length)
          ]
        ...
        const contentTemplate =
          _contentTemplates[
            Math.floor(Math.random() * _contentTemplates.length)
          ]
